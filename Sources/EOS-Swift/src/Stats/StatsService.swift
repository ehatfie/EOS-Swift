...
extension StatServiceValuesProtocol {
  ...
  public var agilityFactor: Double? {
    ...
    guard let agility = ship.attributes![AttrId.agility.rawValue],
          let mass = ship.attributes![AttrId.mass.rawValue] else {
      return nil
    }
...
//    }
  ...
  public func getSlotStats(container: any ItemContainerBaseProtocol, attrId: AttrId) -> SlotStats {
    ...
    let total = Int(self.fit?.ship?.attributes![attrId.rawValue] ?? 0)
